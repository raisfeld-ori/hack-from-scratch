...
def create_level(level: Level):
    ...
    def submit_secret_flag():
        return "Shhh, don't tell anyone. The flag is CTF(IamAHacker)"
    
    @bp.post('/submit_final')
    def submit_final_flag():
        submitted_flag = request.form.get("flag")
        if submitted_flag == "CTF(IamAHacker)":
            response = make_response(redirect('/level-2/'))
            response.set_cookie("current_flag", "CTF(IamAHacker)")
            return response
        else:
